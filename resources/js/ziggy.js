const Ziggy = {
    url: "http:\/\/arkscan.test",
    port: null,
    defaults: {},
    routes: {
        login: { uri: "login", methods: ["GET", "HEAD"] },
        "login.store": { uri: "login", methods: ["POST"] },
        logout: { uri: "logout", methods: ["POST"] },
        "password.confirm": { uri: "user\/confirm-password", methods: ["GET", "HEAD"] },
        "password.confirmation": { uri: "user\/confirmed-password-status", methods: ["GET", "HEAD"] },
        "password.confirm.store": { uri: "user\/confirm-password", methods: ["POST"] },
        "wysiwyg.twitter": { uri: "wysiwyg\/twitter-embed-code", methods: ["GET", "HEAD"] },
        "wysiwyg.upload-image": { uri: "wysiwyg\/upload-image", methods: ["POST"] },
        "wysiwyg.count-characters": { uri: "wysiwyg\/count-characters", methods: ["POST"] },
        "cropper.upload-image": { uri: "cropper\/upload-image", methods: ["POST"] },
        "debugbar.openhandler": { uri: "_debugbar\/open", methods: ["GET", "HEAD"] },
        "debugbar.clockwork": { uri: "_debugbar\/clockwork\/{id}", methods: ["GET", "HEAD"], parameters: ["id"] },
        "debugbar.telescope": { uri: "_debugbar\/telescope\/{id}", methods: ["GET", "HEAD"], parameters: ["id"] },
        "debugbar.assets.css": { uri: "_debugbar\/assets\/stylesheets", methods: ["GET", "HEAD"] },
        "debugbar.assets.js": { uri: "_debugbar\/assets\/javascript", methods: ["GET", "HEAD"] },
        "debugbar.cache.delete": {
            uri: "_debugbar\/cache\/{key}\/{tags?}",
            methods: ["DELETE"],
            parameters: ["key", "tags"],
        },
        "dusk.login": {
            uri: "_dusk\/login\/{userId}\/{guard?}",
            methods: ["GET", "HEAD"],
            parameters: ["userId", "guard"],
        },
        "dusk.logout": { uri: "_dusk\/logout\/{guard?}", methods: ["GET", "HEAD"], parameters: ["guard"] },
        "dusk.user": { uri: "_dusk\/user\/{guard?}", methods: ["GET", "HEAD"], parameters: ["guard"] },
        "horizon.stats.index": { uri: "horizon\/api\/stats", methods: ["GET", "HEAD"] },
        "horizon.workload.index": { uri: "horizon\/api\/workload", methods: ["GET", "HEAD"] },
        "horizon.masters.index": { uri: "horizon\/api\/masters", methods: ["GET", "HEAD"] },
        "horizon.monitoring.index": { uri: "horizon\/api\/monitoring", methods: ["GET", "HEAD"] },
        "horizon.monitoring.store": { uri: "horizon\/api\/monitoring", methods: ["POST"] },
        "horizon.monitoring-tag.paginate": {
            uri: "horizon\/api\/monitoring\/{tag}",
            methods: ["GET", "HEAD"],
            parameters: ["tag"],
        },
        "horizon.monitoring-tag.destroy": {
            uri: "horizon\/api\/monitoring\/{tag}",
            methods: ["DELETE"],
            wheres: { tag: ".*" },
            parameters: ["tag"],
        },
        "horizon.jobs-metrics.index": { uri: "horizon\/api\/metrics\/jobs", methods: ["GET", "HEAD"] },
        "horizon.jobs-metrics.show": {
            uri: "horizon\/api\/metrics\/jobs\/{id}",
            methods: ["GET", "HEAD"],
            parameters: ["id"],
        },
        "horizon.queues-metrics.index": { uri: "horizon\/api\/metrics\/queues", methods: ["GET", "HEAD"] },
        "horizon.queues-metrics.show": {
            uri: "horizon\/api\/metrics\/queues\/{id}",
            methods: ["GET", "HEAD"],
            parameters: ["id"],
        },
        "horizon.jobs-batches.index": { uri: "horizon\/api\/batches", methods: ["GET", "HEAD"] },
        "horizon.jobs-batches.show": {
            uri: "horizon\/api\/batches\/{id}",
            methods: ["GET", "HEAD"],
            parameters: ["id"],
        },
        "horizon.jobs-batches.retry": {
            uri: "horizon\/api\/batches\/retry\/{id}",
            methods: ["POST"],
            parameters: ["id"],
        },
        "horizon.pending-jobs.index": { uri: "horizon\/api\/jobs\/pending", methods: ["GET", "HEAD"] },
        "horizon.completed-jobs.index": { uri: "horizon\/api\/jobs\/completed", methods: ["GET", "HEAD"] },
        "horizon.silenced-jobs.index": { uri: "horizon\/api\/jobs\/silenced", methods: ["GET", "HEAD"] },
        "horizon.failed-jobs.index": { uri: "horizon\/api\/jobs\/failed", methods: ["GET", "HEAD"] },
        "horizon.failed-jobs.show": {
            uri: "horizon\/api\/jobs\/failed\/{id}",
            methods: ["GET", "HEAD"],
            parameters: ["id"],
        },
        "horizon.retry-jobs.show": { uri: "horizon\/api\/jobs\/retry\/{id}", methods: ["POST"], parameters: ["id"] },
        "horizon.jobs.show": { uri: "horizon\/api\/jobs\/{id}", methods: ["GET", "HEAD"], parameters: ["id"] },
        "horizon.index": {
            uri: "horizon\/{view?}",
            methods: ["GET", "HEAD"],
            wheres: { view: "(.*)" },
            parameters: ["view"],
        },
        "livewire.update": { uri: "livewire\/update", methods: ["POST"] },
        "livewire.upload-file": { uri: "livewire\/upload-file", methods: ["POST"] },
        "livewire.preview-file": {
            uri: "livewire\/preview-file\/{filename}",
            methods: ["GET", "HEAD"],
            parameters: ["filename"],
        },
        "ignition.healthCheck": { uri: "_ignition\/health-check", methods: ["GET", "HEAD"] },
        "ignition.executeSolution": { uri: "_ignition\/execute-solution", methods: ["POST"] },
        "ignition.updateConfig": { uri: "_ignition\/update-config", methods: ["POST"] },
        home: { uri: "\/", methods: ["GET", "HEAD"] },
        validators: { uri: "validators", methods: ["GET", "HEAD"] },
        "validator-monitor": { uri: "validator-monitor", methods: ["GET", "HEAD"] },
        blocks: { uri: "blocks", methods: ["GET", "HEAD"] },
        block: { uri: "blocks\/{block}", methods: ["GET", "HEAD"], parameters: ["block"], bindings: { block: "hash" } },
        transactions: { uri: "transactions", methods: ["GET", "HEAD"] },
        transaction: {
            uri: "transactions\/{transaction}",
            methods: ["GET", "HEAD"],
            parameters: ["transaction"],
            bindings: { transaction: "hash" },
        },
        "top-accounts": { uri: "top-accounts", methods: ["GET", "HEAD"] },
        wallet: {
            uri: "addresses\/{wallet}",
            methods: ["GET", "HEAD"],
            parameters: ["wallet"],
            bindings: { wallet: "address" },
        },
        "wallet.blocks": {
            uri: "addresses\/{wallet}?view=blocks",
            methods: ["GET", "HEAD"],
            parameters: ["wallet"],
            bindings: { wallet: "address" },
        },
        "wallet.voters": {
            uri: "addresses\/{wallet}?view=voters",
            methods: ["GET", "HEAD"],
            parameters: ["wallet"],
            bindings: { wallet: "address" },
        },
        "wallet-old": {
            uri: "addresses-old\/{wallet}",
            methods: ["GET", "HEAD"],
            parameters: ["wallet"],
            bindings: { wallet: "address" },
        },
        statistics: { uri: "statistics", methods: ["GET", "HEAD"] },
        contact: { uri: "support", methods: ["GET", "HEAD"] },
        "compatible-wallets": { uri: "compatible-wallets", methods: ["GET", "HEAD"] },
        exchanges: { uri: "exchanges", methods: ["GET", "HEAD"] },
        webhooks: { uri: "webhooks", methods: ["POST"] },
        "currency.update": { uri: "currency\/update", methods: ["POST"] },
        "theme.update": { uri: "theme\/update", methods: ["POST"] },
        "navbar-search.index": { uri: "navbar\/search", methods: ["GET", "HEAD"] },
        "navbar-search.redirect": { uri: "navbar\/search\/redirect", methods: ["POST"] },
    },
};
if (typeof window !== "undefined" && typeof window.Ziggy !== "undefined") {
    Object.assign(Ziggy.routes, window.Ziggy.routes);
}
export { Ziggy };
