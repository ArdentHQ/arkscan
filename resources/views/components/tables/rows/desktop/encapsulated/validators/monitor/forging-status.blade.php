        ...
        <div @class([
            ...
            @if ($model->isPending())
                @if ($withTime)
                    ...
                    />
                @else
                    @lang('tables.validator-monitor.forging-status.pending')
                @endif
            @elseif ($model->hasForged())
                @lang('tables.validator-monitor.forging-status.block_generated')
            @elseif ($model->justMissed())
                @lang('tables.validator-monitor.forging-status.blocks_missed', ['count' => $model->missedCount()])
            @else
                @lang('tables.validator-monitor.forging-status.generating')
            @endif
        </div>
    @endif
</x-general.badge>
