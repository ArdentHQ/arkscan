...
>
    ...
    <div>
        ...
        <div class="flex flex-col md:hidden">
            @foreach ($years as $year)
                <x-stats.insights.container :title="$year['year']" full-width>
                    <div class="flex flex-1 flex-col justify-between pt-3 space-y-3 w-full sm:flex-row sm:space-y-0">
                ...
                <div class="flex flex-col space-y-4">
                    @foreach ($years as $year)
                        <div class="flex">
                            ...
                            <div class="flex flex-1 md-lg:flex-2 flex-col md-lg:flex-row space-y-3 md-lg:space-y-0">
        ...
        <div class="hidden xl:block">
            <x-stats.insights.container full-width>
                <table class="justify-between w-full border-spacing-y-3 border-separate">
                    <tbody>
                        @foreach ($years as $year)
                            <tr>
                                <td>
                                    <div class="text-theme-secondary-900 dark:text-theme-dark-50 pr-8">
                                        {{ $year['year'] }}
                                    </div>
                                </td>
                                <td>
                                    <div class="flex justify-between space-x-3 px-8">
                                        <span>@lang('pages.statistics.insights.annual.header.transaction'):</span>
                                        <span class="text-theme-secondary-900 dark:text-theme-dark-50">
                                            <x-number>{{ $year['transactions'] }}</x-number>
                                        </span>
                                ...
                                <td>
                                    <div class="flex justify-between space-x-3 pl-8">
                                        <span>@lang('pages.statistics.insights.annual.header.blocks'):</span>
                                        <span class="text-theme-secondary-900 dark:text-theme-dark-50">
                                            <x-number>{{ $year['blocks'] }}</x-number>
                                        </span>
