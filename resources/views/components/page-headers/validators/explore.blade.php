...
>
    <x-slot name="background" class="left-0 right-auto sm:right-0 sm:left-auto">
        <img
            src="{{ Vite::image('validators/header-bg.svg') }}"
            class="hidden max-w-none sm:block dark:hidden"
        />

        <img
            src="{{ Vite::image('validators/header-bg-mobile.svg') }}"
            class="max-w-none sm:hidden dark:hidden"
        />

        <img
            src="{{ Vite::image('validators/header-bg-dark.svg') }}"
            class="hidden max-w-none dim:sm:hidden dark:sm:block"
        />

        <img
            src="{{ Vite::image('validators/header-bg-mobile-dark.svg') }}"
            class="hidden max-w-none dark:block dim:hidden dark:sm:hidden"
        />

        <img
            src="{{ Vite::image('validators/header-bg-dim.svg') }}"
            class="hidden max-w-none dark:hidden dim:sm:block"
        />

        <img
            src="{{ Vite::image('validators/header-bg-mobile-dim.svg') }}"
    ...
    <div class="flex relative z-30 flex-col flex-1 items-center space-y-3 sm:flex-row sm:justify-between sm:space-y-0">
        ...
        <div class="w-full sm:w-auto">
            <x-ark-external-link
                ...
                icon-size="sm"
            >
                <span>@lang('pages.validators.explore.action')</span>
            </x-ark-external-link>
        </div>
    </div>
</x-page-headers.header-item>
