            ...
            $query->select('recipient_amount', 'is_multipayment')
                ->from(function ($query) {
                    $query->selectRaw('CAST(encode(SUBSTRING(transactions.data, 69, 32), \'hex\') AS int) as tx_count')
                        ->selectRaw('CASE WHEN to = ? AND encode(SUBSTRING(data FROM 1 FOR 4), \'hex\') = ? THEN TRUE ELSE FALSE END as is_multipayment', [Network::knownContract('multipayment'), ContractMethod::multiPayment()]);
                }, 'd')
