...
trait CanBeSorted
{
    ...
    public function scopeSortByAddress(Builder $query, SortDirection $sortDirection): Builder
    {
        return $query->join('wallets', 'wallets.public_key', '=', 'transactions.sender_public_key')
            ->orderBy('wallets.address', $sortDirection->value);
    }

    public function scopeSortByType(Builder $query, SortDirection $sortDirection): Builder
    {
        return $query->select([
